 ...
 * @param  g [description]
 */
void snapshot(Graph *g); // XXX unused
/* Pseudocode:
 ...
 * @return   [description]
 */
void upsert(Graph *g, Vertex *v); // XXX unused
/* Pseudocode:
...
LogEntry *create_log(Graph *, ID);
LogEntry *get_log(Graph *, ID);
...
// called by worker threads to cooperate with snapshot thread once they are done
// inserting vertices (ie, when they have no other job to do)
